...
UIwndClass, baseClass = uic.loadUiType('%s/HeadStreatchTool.ui'%getScriptPath())
...
def makeHeadStreatch(jointCount=5):
    ...
    mc.setAttr('%s.radius'%moreWeghtJnt, 2)
    # make streatch
    ...
    # Rig
    BindCurveStartJnt = mc.createNode('joint')
    BindCurveEndJnt   = mc.createNode('joint')
    
    
    mc.delete(mc.parentConstraint(Joints[0],  BindCurveStartJnt))
    mc.delete(mc.parentConstraint(Joints[-1], BindCurveEndJnt))
    
    # bind Curve
    mc.skinCluster(BindCurveStartJnt, BindCurveEndJnt, attachCurve)
    mc.skinCluster(BindCurveStartJnt, BindCurveEndJnt, upVectorCurve)
    mc.skinCluster(BindCurveStartJnt, baseDistanceCurve)
    ...
    endBindJntUpperLocator = mc.spaceLocator(p=(0,0,0))[0]
    
    # match position
    mc.delete(mc.parentConstraint(BindCurveStartJnt, localStartControlGrp))
    mc.delete(mc.parentConstraint(BindCurveEndJnt,   localEndControlGrp))   
    
    mc.delete(mc.parentConstraint(BindCurveStartJnt, startControlGrp))
    mc.delete(mc.parentConstraint(BindCurveEndJnt,   endControlGrp))  
    
    mc.delete(mc.parentConstraint(BindCurveEndJnt,   endBindJntUpperLocator))  
    mc.move(mc.arclen(upVectorCurve, ch=False) / 18, 0, 0, endBindJntUpperLocator, r=True)
    ...
    # constraint 
    mc.aimConstraint(BindCurveStartJnt, BindCurveEndJnt, aimVector=(0, -1, 0), upVector=(1, 0, 0), worldUpType='object', worldUpObject=endBindJntUpperLocator)

    
    # Compile
    mc.parent(BindCurveStartJnt, localStartControl)
    mc.parent(BindCurveEndJnt, localEndControl)
    mc.parent(endBindJntUpperLocator, localEndControl)
    ...
    setGrp     = mc.group(JointGrp, LocatorGrp, CurveGrp)
    ...
    # - bind curve joint -
    BindCurveStartJnt      = mc.rename(BindCurveStartJnt, 'C_HSstartBindCus_jnt_0')
    BindCurveEndJnt        = mc.rename(BindCurveEndJnt, 'C_HSendBindCus_jnt_0')
    endBindJntUpperLocator = mc.rename(endBindJntUpperLocator, 'C_HSendbindcusJntup_loc_0')
